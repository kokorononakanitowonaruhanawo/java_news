...
<body>
    ...
    <div class="container">
        ...
        <div class="row my-2">
            ...
            <div class="col-sm-8">
                <form action="GenreRegisterServlet" method="post">
                    <legend>ジャンル（登録）</legend>
                     <!-- 登録済みジャンル -->
                    <div class="row mb-5">
                        <label for="complete" class="col-sm-3 col-form-label">登録済みジャンル</label>
                        <div class="col-sm-9">
                            <select class="form-select" aria-label="complete">
                       			<option selected>必要に応じて確認してください</option>
                       			<c:forEach var="genre" items="${sessionScope.genreList}">
                       				<option value="${genre.getId()}">${genre.getGenre()}</option>
                       			</c:forEach>
                            </select>
                            <div class="form-text">任意</div>
                        </div>
                    </div>
                    <!-- 新規追加ジャンル -->
                    <div class="row mb-3">
                        <label for="genre" class="col-sm-3 col-form-label">新規登録ジャンル</label>
                        <div class="col-sm-9">
                            <input type="text"
                            		id="genre" name="genre"
